...
from src.utils import ListConcat, serialize
...
class Trainer:
    def __init__(self, graph: KG, train_facts, cutoff, reverse_relation: list,
        ...
        if test_tasks is not None:
            test_support, tail_test_eval, head_test_eval = test_tasks
            self.tail_test_relations, self.head_test_relations = [], []
            for head, relation, tail in tail_test_eval:
                if relation in self.train_query and relation not in self.tail_test_relations:
                    self.tail_test_relations.append(relation)
                    self.test_support[relation] = None
                self.e1rel2_e2_test.setdefault((head, relation), set())
                self.e1rel2_e2_test[(head, relation)].add(tail)
                self.test_ground.setdefault(relation, [])
                self.test_ground[relation].append((head, tail))
            for head, relation, tail in head_test_eval:
                if relation in self.train_query and relation not in self.head_test_relations:
                    self.head_test_relations.append(relation)
                    self.test_support[relation] = None
                self.e1rel2_e2_test.setdefault((head, relation), set())
                self.e1rel2_e2_test[(head, relation)].add(tail)
                self.test_ground.setdefault(relation, [])
                self.test_ground[relation].append((head, tail))
            self.test_relations = self.head_test_relations + self.tail_test_relations
            
        self.valid_ground, self.valid_support = {}, {}
        if validate_tasks is not None:
            valid_support, tail_valid_eval, head_valid_eval = validate_tasks
            self.tail_validate_relations, self.head_validate_relations = [], []
            for head, relation, tail in tail_valid_eval:
                if relation in self.train_query and relation not in self.tail_validate_relations:
                    self.tail_validate_relations.append(relation)
                    self.valid_support[relation] = None
                self.e1rel2_e2_test.setdefault((head, relation), set())
                self.e1rel2_e2_test[(head, relation)].add(tail)
                self.valid_ground.setdefault(relation, [])
                self.valid_ground[relation].append((head, tail))
            for head, relation, tail in head_valid_eval:
                if relation in self.train_query and relation not in self.head_validate_relations:
                    self.head_validate_relations.append(relation)
                    self.valid_support[relation] = None
                self.e1rel2_e2_test.setdefault((head, relation), set())
                self.e1rel2_e2_test[(head, relation)].add(tail)
                self.valid_ground.setdefault(relation, [])
                self.valid_ground[relation].append((head, tail))
            self.validate_relations = self.head_validate_relations + self.tail_validate_relations
    ...
    def sample(self, batch_size, specific_relation=None):
        ...
        for idx in range(batch_size):
            ...
            if self.meta_learn:
                ...
                query_pair = random.choice(self.train_query[relation])
                # query_pair = self.train_query[relation][0]
            if self.ignore_onehop:
    ...
    def evaluate(self, specific_relation=None, mode='test', part='tail'):
        self.graph.eval()
        if specific_relation is None:
            if mode == 'test':
                if part == 'tail':
                    evaluate_relations, evaluate_support, evaluate_ground = self.tail_test_relations, self.test_support, self.test_ground
                else:
                    evaluate_relations, evaluate_support, evaluate_ground = self.head_test_relations, self.test_support, self.test_ground
            elif mode == 'valid':
                if part == 'tail':
                    evaluate_relations, evaluate_support, evaluate_ground = self.tail_validate_relations, self.valid_support, self.valid_ground
                else:
                    evaluate_relations, evaluate_support, evaluate_ground = self.head_validate_relations, self.valid_support, self.valid_ground
            else:
    ...
    def evaluate_generator(self, module, data_loader, batch_size=16, save_result=None, save_graph=None, eval_mode='mult'):
        if save_result:
            save_result = open(save_result, "w")
        if save_graph:
            self.reason_graphs = {}
        self.correct_cases = []
        for idx, (relation_id, support_pair, evaluate_data, ground_sets) in enumerate(tqdm(data_loader)):
            # print('relation: {}'.format(relation_id))
            # if relation_id == 93:
            #     print('break')
            ...
            for idx_batch in BatchSampler(SequentialSampler(evaluate_data), batch_size=batch_size, drop_last=False):
                ...
                if self.meta_learn:
                    scores = module(start_entities, other_correct_answers=other_correct_answers,
                                             ...
                                             candidates=cur_candidates, end_entities=tail_entities, do_sample=False)
                else:
                    relations = [relation_id] * len(start_entities)
                    scores = module(start_entities, other_correct_answers=other_correct_answers, 
                                    relations=relations, evaluate=True, 
                                    candidates=cur_candidates, end_entities=tail_entities, do_sample=False, eval_mode=eval_mode)
                ...
                # prior_scores = prior_scores.reshape((self.test_rollout_num, len(batch), entity_num)).sum(dim=0)
                scores = scores[:, rand_idxs]
                for batch_id in range(len(batch)):
                    score = scores[batch_id]
                    score[list(ground[batch_id])] -= 1.0
                    _, ranked = score.sort(dim=-1, descending=True)
                    ranking = (entity_list[ranked] == batch[batch_id][1]).nonzero().item()
                    if ranking == 0:
                        self.correct_cases.append("\t".join((self.id2entity[start_entities[batch_id]], 
                                                          self.id2relation[relation_id], 
                                                          self.id2entity[tail_entities[batch_id]])))
                        if save_graph:
                            reason_paths = module.get_correct_path(batch_id, [batch[batch_id][1]], return_graph=True)
                            self.reason_graphs["\t".join((self.id2entity[start_entities[batch_id]], 
                                                          self.id2relation[relation_id], 
                                                          self.id2entity[tail_entities[batch_id]]))] = reason_paths
                    yield [batch[batch_id][1]], ranking
        if save_result:
            save_result.close()
        if save_graph:
